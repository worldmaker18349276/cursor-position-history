...
class History {
    ...
    push(position) {
        ...
        // DEBUG: this.editor.decorateMarker(marker, {type: 'cursor', class: "cursor-history-debug"});
        this.history.push(marker);
        this.index = this.history.length - 1;

        // trim history size
        const size = atom.config.get("cursor-history.maxHistorySize")
...
module.exports = {
    ...
    activate() {
        ...
        this.disposables.add(atom.commands.add('atom-workspace', {
            'cursor-history:previous': () => this.previous(),
            'cursor-history:next': () => this.next(),
            'cursor-history:push': () => this.push(),
            'cursor-history:clear': () => this.clear(),
        }));
    },
