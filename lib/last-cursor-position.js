...
const DEBOUNCE_TIME = 500.0;
const DEBOUNCE_DIS = 5;
const DEBUG = true;
...
class History {
    ...
    update(event) {
        ...
        if (delta < DEBOUNCE_TIME) {
            ...
            const pos = this.last();
            if (pos !== null && Math.abs(event.newBufferPosition.row - pos.row) <= DEBOUNCE_DIS) {
                return;
            }
        }
    ...
    push(position) {
        ...
        if (DEBUG) {
            this.editor.decorateMarker(marker, {type: 'cursor', class: "last-cursor-position-debug"});
        }
