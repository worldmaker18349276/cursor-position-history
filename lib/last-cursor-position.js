...
module.exports = {
    ...
    push() {
        const activeEd = atom.workspace.getActiveTextEditor();
        const activePane = atom.workspace.getActivePane();
        this.history.splice(this.index, 0, {
            pane: activePane,
            editor: activeEd,
    ...
    previous() {
        ...
        if (this.state !== STATE.REWINDING && this.index > 0) {
            this.index -= 1;
        }
        ...
        const pos = this.history[this.index - 1];
    ...
    next() {
        ...
        // when changing direction, we need to store last position, but not move to it
        if (this.state !== STATE.FORWARDING && this.index < this.history.length) {
