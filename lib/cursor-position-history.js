...
class HistoryManager {
    ...
    trim() {
        if (this.markers.length <= this.maxSize) {
            return;
        }

        const numToTrim = this.markers.length - this.maxSize;
        if (numToTrim <= this.index) {
            for (const marker of this.markers.splice(0, numToTrim)) {
                marker.destroy();
            }
            this.index = this.index - numToTrim;

        } else {
            // don't remove current position
            for (const marker of this.markers.splice(this.index+1, numToTrim-this.index)) {
                marker.destroy();
            }
            for (const marker of this.markers.splice(0, this.index)) {
                marker.destroy();
            }
            this.index = 0;
        }
...
class CursorTraveler {
    constructor(editor) {
        this.editor = editor;
        const maxSize = atom.config.get("cursor-position-history.maxHistorySize");
        this.history = new HistoryManager(editor, maxSize);
    ...
    onCursorMove(event) {
        ...
        if (!isMoving) {
            ...
            // console.assert(this.history.current() !== null);
            const currentPosition = this.editor.screenPositionForBufferPosition(this.history.current());
            if (Math.abs(event.newScreenPosition.row - currentPosition.row) <= DEBOUNCE_DIS) {
                return;
            }
        }
    ...
    onSizeChange(event) {
        this.history.maxSize = event.newValue;
        this.history.trim();
    }
...
module.exports = {
    ...
    initializeCursorTraveler(editor) {
        ...
        editor_subscriptions.add(atom.config.onDidChange("cursor-position-history.maxHistorySize", event => traveler.onSizeChange(event)));
    ...
    previous() {
        this.getActiveCursorTraveler().moveToPastPosition();
    },

    next() {
        this.getActiveCursorTraveler().moveToFuturePosition();
    },

    push() {
        this.getActiveCursorTraveler().recordCurrentPosition();
    },

    clear() {
        this.getActiveCursorTraveler().clearHistory();
    },
};
